...
BOOST_AUTO_TEST_CASE(SimpleTreeParseKey) {
  ...
  const auto res_float = SimpleTree::parse_key("(3.141)");
  BOOST_REQUIRE_EQUAL(res_float.size(), 1);
  BOOST_CHECK_EQUAL(res_float[0], 3.141);

  const auto res_floats = SimpleTree::parse_key("(4.3,2.1)");
  ...
  BOOST_CHECK_THROW(SimpleTree::parse_key("1"), base_error);
  BOOST_CHECK_THROW(SimpleTree::parse_key(")"), base_error);
  BOOST_CHECK_THROW(SimpleTree::parse_key("("), base_error);
  BOOST_CHECK_THROW(SimpleTree::parse_key("(1"), base_error);
  BOOST_CHECK_THROW(SimpleTree::parse_key("(2,"), base_error);
  BOOST_CHECK_THROW(SimpleTree::parse_key("(2,1"), base_error);
}
