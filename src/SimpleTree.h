...
private:
    ...
    struct node_t : public std::enable_shared_from_this<node_t> {
        ...
        bool check_tiles(node_t* start, std::vector<std::shared_ptr<node_t>>& , std::vector<std::pair<double,double>>& bounds, double val, double minval, bool minimization, size_t offset);
