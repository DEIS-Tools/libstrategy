...
void SimpleTree::node_t::subsumption_reduction(bool minimization, SimpleTree& parent) 
{
    ...
    else 
    {
        ...
        while(true)
        {
            ...
            for(auto& n : nodes)
            {
                outer |= n->check_tiles(n.get(), nodes, bounds, val, minval, maxval, minimization, parent._statevars.size() + 1);
            }
...
/*        std::cerr << "HANDLE " << std::endl;
           ...
           std::isinf(_low->_cost) == std::isinf(_high->_cost))
        {
            ...
            print(std::cerr);
            std::cerr << std::endl;*/
            _cost = _low->midcost(*_high, minval, maxval);
        ...
        if(false) {
                ...
                std::isinf(_low->_cost) == std::isinf(_high->_low->_cost))
            {
                node = _high;
                other = _high->_low;
                nc = other->midcost(*_low, minval, maxval);
                assert(!std::isnan(nc));
            } 
                      ...
                      std::isinf(_high->_cost) == std::isinf(_low->_high->_cost))
            {
                node = _low;
                other = _low->_high;
                nc = other->midcost(*_high, minval, maxval);
                assert(!std::isnan(nc));
            }
               ...
               std::isinf(_low->_high->_cost) == std::isinf(_high->_low->_cost))
            {
                ...
                _high->_low->_cost = _low->_high->midcost(*_high->_low, minval, maxval);
                _var = _low->_var;
                _limit = _low->_limit;
                _low = _low->_low;
                // we could compute the values on the fly; but no time currently
