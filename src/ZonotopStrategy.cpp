    ...
    if (raw.is_object()) {
        for (auto& [raw_key, raw_value] : raw.items()) {
            ...
            if (raw_value.is_array()) {
                ...
                throw base_error("Input JSON not well formatted (array field expected)");
            }
        }
    } else {
...
void ZonotopStrategy::try_merge(node_t* node, size_t state)
{
    ...
    if (hlb != std::end(node->_high_patterns) && *hlb == state) {
        ...
        if (llb != std::end(node->_low_patterns) && *llb == state) {
            ...
            bool empty = (node->_high == nullptr && node->_low == nullptr && node->_high_patterns.empty() &&
                          node->_low_patterns.empty());
...
double ZonotopStrategy::get_max(size_t dimen) const { return _root->get_max(dimen); }
