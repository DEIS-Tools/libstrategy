...
void destroy_strategy(void* ptr)
{
    ...
    delete static_cast<ZonotopStrategy*>(ptr);
}

int num_patterns(void* ptr)
{
    std::cerr << "num_patterns " << ptr << std::endl;
    auto pl = static_cast<const ZonotopStrategy*>(ptr)->num_patterns();
...
int max_pattern_length(void* ptr)
{
    ...
    auto pl = static_cast<const ZonotopStrategy*>(ptr)->max_pattern_length();
    std::cerr << pl << std::endl;
    return pl;
}
...
void active(void* ptr, const double* sample, bool* write)
{
    ...
    static_cast<const ZonotopStrategy*>(ptr)->active(sample, write);
}

int get_pattern(void* ptr, int el, int* write)
{
    //    std::cerr << "get_pattern " << ptr << std::endl;
    return static_cast<ZonotopStrategy*>(ptr)->get_pattern(el, write);
}

double get_min(void* ptr, int dimen)
{
    auto val = static_cast<const ZonotopStrategy*>(ptr)->get_min(dimen);
...
{
    auto val = static_cast<const ZonotopStrategy*>(ptr)->get_max(dimen);
...
void destroy_learned(void* ptr) { delete static_cast<SimpleTree*>(ptr); }

double weight(void* ptr, const double* disc, const double* cont, int a)
{
    return static_cast<const SimpleTree*>(ptr)->value(disc, cont, a);
}
