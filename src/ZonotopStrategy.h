...
private:
  ...
  struct node_t : std::enable_shared_from_this<node_t> {
    ...
    std::ostream &print(std::ostream &os, const ZonotopStrategy *parent,
                        size_t tabs = 0) const;
  ...
  struct bound_t {
    ...
    double _upper = 0;
    bound_t() = default;
    bound_t(double l, double u) : _lower{l}, _upper{u} {}
  };
  static std::vector<bound_t> get_bounds(const std::string &zonotop,
                                         size_t &vars);
  bool add(std::vector<bound_t> &bounds, size_t state);
  bool rec_insert(node_t *node, std::vector<bound_t> &bounds,
